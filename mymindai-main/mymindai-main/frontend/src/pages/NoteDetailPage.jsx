...
import { updateNote, deleteNote } from '../store/note';
import Modal from '../components/Modal';
import './NoteDetailPage.css';

const NoteDetailPage = ({ notes, onUpdate }) => {
  const { id } = useParams();
  ...
  if (!note) {
    return <div className="note-not-found">Заметка не найдена</div>;
  }
  ...
  const handleSave = async () => {
    ...
    const response = await updateNote({title: editedTitle, content: updatedContent.replace(/<br>/g, '\n').replace(/<\/li>\n<li>/g, '</li><li>')}, note.id);
  ...
  return (
        ...
        {isEditing ? (
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: formatContentForDisplay(note.content)
            }}
          />
        )}
      </div>
    ...
    >
      ...
      <div className='event-actions'>
        ...
        <Button style={{ height: 50}} type="primary" block onClick={() => removeNote(note.id)}>
          Да
        </Button>
      </div>
    </Modal>
